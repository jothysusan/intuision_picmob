...
public class RecyclerViewItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public RecyclerViewItemDecoration(Context con, int headerHeight, boolean isSticky, SectionCallback callback) {
        context = con;
    ...
    private void drawHeader(Canvas c, View child, View headerView) {
        c.save();
        if (sticky) {
            c.translate(0, Math.max(0, child.getTop() - (headerView.getHeight() + 50)));
        } else {
    ...
    private View inflateHeader(RecyclerView recyclerView) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_header, recyclerView, false);
        return view;
    }

...
import com.picmob.android.chat.DeviceConnectionState
...
object ChatServer {
    private var pojo: UserAuthPojo? = null
    ...
    fun startServer(app: Application) {
        pojo = Gson().fromJson(
            CustomSharedPreference.getInstance(ChatServer.app)
                .getString(AppConstants.USR_DETAIL), UserAuthPojo::class.java
        )
    ...
    fun sendMessage(message: String): Boolean {
        ...
        messageCharacteristic?.let { characteristic ->
            ...
            gatt?.let {
                val success = if (ChatServer.app?.let { it1 ->
                        ActivityCompat.checkSelfPermission(
                            it1,
                            Manifest.permission.BLUETOOTH_CONNECT
                        )
                ...
                it.writeCharacteristic(messageCharacteristic)
    ...
    private fun startAdvertisement() {
        if (ChatServer.app?.let {
            ...
            // to handle the case where the user grants the permission. See the documentation
            // for ActivityCompat#requestPermissions for more details.
            adapter.setName(pojo?.username + "_" + pojo?.id)
    ...
    private class GattClientCallback : BluetoothGattCallback() {
        ...
        override fun onServicesDiscovered(discoveredGatt: BluetoothGatt, status: Int) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                Toast.makeText(ChatServer.app, "Discovered", Toast.LENGTH_LONG).show()
            }
        }
    }
